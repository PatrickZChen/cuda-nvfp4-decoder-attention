...
python_executable="${CUDA_RMSNORM_PYTHON:-${repository_root}/.venv/bin/python}"
if [[ -n "${CUDA_RMSNORM_CMAKE:-}" ]]; then
    cmake_executable="${CUDA_RMSNORM_CMAKE}"
elif [[ -x "${repository_root}/.venv/bin/cmake" ]]; then
    cmake_executable="${repository_root}/.venv/bin/cmake"
else
    cmake_executable="cmake"
fi
build_directory="${CUDA_RMSNORM_BUILD_DIR:-${repository_root}/build-cuda}"
...
"${cmake_executable}" \
    ...
    -DCUDA_RMSNORM_PYTORCH_CUDA_VERSION="${pytorch_cuda_version}" \
