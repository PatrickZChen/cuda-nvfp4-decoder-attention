"""Run M4C grouped-decode W4A16 cases under Compute Sanitizer."""
...
CASES = (
    ("multiple-rows-k32", 3, 5, 32, 40_003),
    ("k128", 2, 8, 128, 40_009),
    ("k3072", 2, 4, 3072, 40_013),
)
...
def _hand_case() -> tuple[float, float, int, int, int]:
    ...
    actual = cuda_primitives.cuda_w4a16_linear_grouped_decode(
        x_cpu.cuda(),
...
def _zero_scale_case() -> tuple[float, float, int, int, int]:
    ...
    return _check("zero-scale-storage", actual, expected_cpu.cuda())
...
def main() -> None:
    ...
    print("grouped_decode_w4a16_kernel_executed=true")
