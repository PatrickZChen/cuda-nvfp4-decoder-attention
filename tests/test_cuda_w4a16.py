"""Correctness and integration tests for Milestone 4A direct W4A16 CUDA."""
...
def _call(x: torch.Tensor, weight: NVFP4Tensor) -> torch.Tensor:
    ...
    return cuda_primitives.cuda_w4a16_linear(x, weight)
...
def test_hand_computable_projection_is_exact() -> None:
    # beta=1 (0x38), gamma=1. Row 0 repeats [1,2], while row 1 repeats
    # [1,-1]. These packed bytes and expected sums are independent of CUDA.
    weight_cpu = _storage(
        [[0x42] * 8, [0xA2] * 8],
        [[0x38], [0x38]],
        1.0,
...
def test_every_e2m1_code_is_decoded_inside_projection() -> None:
    packed_rows = [
        [code | (code << 4)] * 8
        for code in range(16)
    ]
    weight = _move_quantized(
        _storage(packed_rows, [[0x38]] * 16, 1.0)
    )
    x = torch.zeros(16, dtype=torch.bfloat16, device="cuda")
    x[0] = 1.0
    expected = torch.tensor(E2M1_VALUES, dtype=torch.bfloat16, device="cuda")

    actual = _call(x, weight)

    # FP32 accumulation starts at +0, so the decoded -0 row stores numerical
    # zero after addition; every nonzero signed code remains independently
    # observable here.
    assert torch.equal(actual, expected)


def test_every_finite_ue4m3_code_is_decoded_inside_projection() -> None:
...
def test_weight_orientation_is_x_times_w_transpose() -> None:
    weight_cpu = _storage(
        [
            [0x22] * 8,  # sixteen +1 values
            [0x44] * 8,  # sixteen +2 values
            [0xA2] * 8,  # alternating +1, -1
        ],
        [[0x38], [0x38], [0x38]],
        1.0,
    )
...
def test_low_and_high_packed_nibbles_both_participate() -> None:
    # Every byte 0x42 holds q_even=1 in its low nibble and q_odd=2 high.
    weight = _move_quantized(_storage([[0x42] * 8], [[0x38]], 1.0))
    x = torch.zeros((2, 16), dtype=torch.bfloat16)
    x[0, 0::2] = 1.0
    x[1, 1::2] = 1.0
    expected = torch.tensor([[8.0], [16.0]], dtype=torch.bfloat16)

    actual = _call(x.cuda(), weight)

    assert torch.equal(actual.cpu(), expected)


def test_row_local_block_scale_indexing_is_exact() -> None:
    # q=1 everywhere. Each (output row, K-block) has a distinct exact beta.
    weight = _move_quantized(
        _storage(
            [[0x22] * 16, [0x22] * 16],
...
def test_zero_activation_zero_weight_and_zero_block_match_oracle() -> None:
    nonzero_source = torch.linspace(-2.0, 2.0, 64).reshape(2, 32)
    nonzero_weight = _move_quantized(quantize_nvfp4_reference(nonzero_source))
    zero_x = torch.zeros((3, 32), dtype=torch.bfloat16, device="cuda")
    assert torch.equal(
        _call(zero_x, nonzero_weight),
        w4a16_linear_reference(zero_x, nonzero_weight),
    )

    zero_source = torch.zeros((2, 32), dtype=torch.float32)
    zero_weight = _move_quantized(quantize_nvfp4_reference(zero_source))
    x = _deterministic_activation((3, 32), 9_101).cuda()
    assert torch.equal(
        _call(x, zero_weight),
        w4a16_linear_reference(x, zero_weight),
    )

    zero_block_source = torch.zeros((2, 32), dtype=torch.float32)
    zero_block_source[:, 16:] = torch.tensor(
        [1.0, -1.0] * 8,
        dtype=torch.float32,
    )
    zero_block_weight = _move_quantized(
        quantize_nvfp4_reference(zero_block_source)
    )
    assert torch.count_nonzero(zero_block_weight.block_scales[:, 0]) == 0
    actual = _call(x, zero_block_weight)
    expected = w4a16_linear_reference(x, zero_block_weight)
    assert torch.equal(actual, expected)
...
def test_m2b_generated_representative_matrices_are_within_one_bf16(
    activation_shape: tuple[int, ...],
    output_features: int,
) -> None:
    reduction_size = activation_shape[-1]
    seed = 10_001 + output_features * 101 + reduction_size
    ...
    print(
        f"representative shape={activation_shape} n={output_features} "
        ...
        "max_bf16_distance="
        f"{metrics.maximum_bf16_adjacency_distance}"
    )
    ...
    assert 0.0 <= metrics.exact_bf16_fraction <= 1.0


def test_multiple_rows_with_k3072_match_reference() -> None:
    source = _deterministic_source(24, 3072, 11_003)
    weight = _move_quantized(quantize_nvfp4_reference(source))
    x = _deterministic_activation((1, 2, 3072), 11_009).cuda()
    ...
    print(
        "long-reduction shape=(1, 2, 3072) n=24 "
        f"max_abs={metrics.maximum_absolute_error:.9g} "
        f"mean_abs={metrics.mean_absolute_error:.9g} "
        f"exact_fraction={metrics.exact_bf16_fraction:.9g} "
        "max_bf16_distance="
    ...
    assert actual.shape == (1, 2, 24)
    assert metrics.maximum_bf16_adjacency_distance <= 1
...
def test_canonical_decoder_projection_executes_directly(
    label: str,
    output_features: int,
) -> None:
    reduction_size = 3072
    seed = 12_001 + output_features
    ...
    print(
        f"canonical-{label} shape={tuple(actual.shape)} "
        f"max_abs={metrics.maximum_absolute_error:.9g} "
        f"mean_abs={metrics.mean_absolute_error:.9g} "
        f"exact_fraction={metrics.exact_bf16_fraction:.9g} "
        "max_bf16_distance="
...
def test_non_default_current_stream_orders_all_inputs_kernel_and_consumer() -> None:
    source = _deterministic_source(8, 128, 13_003)
    target_weight = _move_quantized(quantize_nvfp4_reference(source))
    target_x = _deterministic_activation((2, 128), 13_007).cuda()
    ...
    metrics = _assert_at_most_one_adjacent_bf16(actual, expected)
    print(
        f"current-stream max_abs={metrics.maximum_absolute_error:.9g} "
        f"mean_abs={metrics.mean_absolute_error:.9g} "
        f"exact_fraction={metrics.exact_bf16_fraction:.9g} "
        "max_bf16_distance="
        f"{metrics.maximum_bf16_adjacency_distance}"
...
def test_python_wrapper_requires_nvfp4_tensor() -> None:
    assert cuda_primitives is not None
    x, packed, _, _ = _valid_raw_storage()
    with pytest.raises(TypeError, match="NVFP4Tensor"):
        cuda_primitives.cuda_w4a16_linear(x, packed)  # type: ignore[arg-type]
...
def test_raw_operator_rejects_invalid_activation_structure(
    case: str,
    message: str,
) -> None:
...
def test_raw_operator_rejects_invalid_packed_weight_structure(
    case: str,
    message: str,
) -> None:
...
def test_raw_operator_rejects_invalid_block_scale_structure(
    case: str,
    message: str,
) -> None:
...
def test_raw_operator_rejects_invalid_global_decode_scale_structure(
    case: str,
    message: str,
) -> None:
...
def test_raw_operator_rejects_device_mismatch_when_testable(field: str) -> None:
