...
constexpr int kWarpsPerBlock =
    kGqaAttentionCachedBlockThreads / kWarpSize;
constexpr unsigned int kFullWarpMask = 0xffffffffU;

static_assert(kGqaAttentionCachedBlockThreads == 256);
static_assert(kGqaAttentionCachedBlockThreads % kWarpSize == 0);
...
__global__ void qk_scores_cached_kernel(
    ...
    std::int64_t logical_context_length,
    ...
    const std::int64_t key_index =
        remaining % logical_context_length;
    remaining /= logical_context_length;
    ...
    // Logical visibility is bounded by S=P+T even though each physical cache
    // head owns C slots. This branch is uniform across the whole block.
    ...
    // C, not logical S, is the physical batch/head stride of [B,Hkv,C,D].
    const std::int64_t k_row_offset =
        ((batch * kv_head_count + kv_head) * cache_capacity + key_index) *
        head_dim;

    float partial_sum = 0.0F;
    for (std::int64_t d = threadIdx.x; d < head_dim; d += blockDim.x) {
        const float q_value = __bfloat162float(q[q_row_offset + d]);
        const float k_value = __bfloat162float(k_cache[k_row_offset + d]);
...
__global__ void softmax_cached_kernel(
    const float* scores,
    float* probabilities,
    ...
    const std::int64_t row_offset = row * logical_context_length;
    __shared__ float warp_values[kWarpsPerBlock];

    float thread_maximum = -CUDART_INF_F;
    for (std::int64_t j = threadIdx.x; j < logical_context_length;
         j += blockDim.x) {
    ...
    // Every row has the visible key j=P+i. There is deliberately no epsilon,
    // clamp, or alternate exponential relative to the frozen M5A semantics.
    for (std::int64_t j = threadIdx.x; j < logical_context_length;
         j += blockDim.x) {
...
__global__ void pv_context_cached_kernel(
    ...
    const std::int64_t probability_row_offset =
        ((batch * query_head_count + query_head) * token_count +
         token_index) * logical_context_length;

    // C, not logical S, remains the physical V head stride.
    const std::int64_t v_head_offset =
        (batch * kv_head_count + kv_head) * cache_capacity * head_dim;
    ...
    for (std::int64_t j = threadIdx.x; j < logical_context_length;
        ...
        const float v_value = __bfloat162float(
            v_cache[v_head_offset + j * head_dim + d]);
...
void launch_gqa_attention_cached_cuda(
    ...
    std::int64_t past_length,
    float inverse_sqrt_head_dim) {
    const std::int64_t logical_context_length = scores.size(3);
    ...
    TORCH_INTERNAL_ASSERT(logical_context_length > 0);
    TORCH_INTERNAL_ASSERT(cache_capacity >= logical_context_length);
    ...
    qk_scores_cached_kernel<<<
        ...
        kGqaAttentionCachedBlockThreads,
        0,
        stream>>>(
        q_data,
        k_data,
